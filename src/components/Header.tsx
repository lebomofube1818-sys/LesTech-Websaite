                ...
                <div key={link.name} className="relative" ref={activeDropdown === link.name ? dropdownRef : null}>
                  {link.isDropdown ? (
                    <div className="relative group/nav">
                      <button
                        className="relative px-4 py-2 text-white/70 hover:text-white transition-colors flex items-center gap-1 cursor-pointer"
                        onMouseEnter={() => {
                      ...
                      >
                        ...
                        >
                          <ChevronDown className={`w-4 h-4 ${hoveredLink === link.name ? 'text-black' : 'text-white/40'}`} />
                        </motion.div>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className="md:hidden fixed top-20 left-6 right-6 glass brand-border rounded-3xl p-8 z-[150] shadow-2xl backdrop-blur-2xl bg-black/80"
          >
              ...
              {navLinks.map((link) => (
                <div key={link.name} className="flex flex-col gap-2">
                  {link.isDropdown ? (
                    <>
                      <button 
                        className="flex items-center justify-between text-2xl font-black text-white/90 hover:text-brand-green transition-all tracking-tighter outline-none"
                        onClick={() => setActiveDropdown(activeDropdown === link.name ? null : link.name)}
                      >
                        {link.name}
                          ...
                          >
                            {link.items?.map((item) => (
                              <Link
                                key={item.name}
                                to={link.href + '#' + item.name.toLowerCase().replace(' ', '-')}
                                className="text-lg font-bold text-white/60 hover:text-brand-green transition-colors py-1"
                                onClick={() => {
                                  setIsOpen(false);
                                  setActiveDropdown(null);
                                }}
